/** 
modified by Shadowdogy 「2021-01-18 20:50」
issue: https://github.com/shadowdogy/QX-Tools/issues
...
let [res_link, res_content, res_info] = [$resource.link, $resource.content, $resource.info];
const subtag = $resource.tag != undefined ? $resource.tag : "";
////// 非 raw 链接的沙雕情形
res_content =
  res_content.indexOf("DOCTYPE html") != -1 && res_link.indexOf("github.com") != -1
    ? ToRaw(res_content)
    : res_content;
//ends 正常使用部分，調試註釋此部分

var para = /^(http|https)\:\/\//.test(res_link) ? res_link : res_content.split("\n")[0];
...
var Preg =
  mark0 && para1.indexOf("regex=") != -1
    ? decodeURIComponent(para1.split("regex=")[1].split("&")[0])
    : null; //server正则过滤参数
var Pregdel =
  mark0 && para1.indexOf("delreg=") != -1
    ? decodeURIComponent(para1.split("delreg=")[1].split("&")[0])
    : null; // 正则删除参数
var Phin0 =
...
function Parser() {
  try {
    type0 = Type_Check(res_content); //  类型判断
    //$notify(type0,"hh")
    total = ResourceParse();
  } catch (err) {
...
function ResourceParse() {
  ...
  if (type0 == "Subs-B64Encode") {
    total = Subs2QX(Base64.decode(res_content), Pudp0, Ptfo0, Pcert0, PTls13);
  } else if (type0 == "Subs") {
    total = Subs2QX(res_content, Pudp0, Ptfo0, Pcert0, PTls13);
  } else if (type0 == "QuanX" || type0 == "Clash" || type0 == "Surge") {
    total = isQuanX(res_content);
    total = Subs2QX(total.join("\n"), Pudp0, Ptfo0, Pcert0, PTls13);
  } else if (type0 == "sgmodule") {
    // surge module 模块/含 url-regex 的 rule-set
    flag = 2;
    total = SGMD2QX(res_content); // 转换
    ...
    // rewrite 类型
    flag = 2;
    total = Rewrite_Filter(isQuanXRewrite(res_content.split("\n")), Pin0, Pout0);
    if (Preplace) {
      total = ReplaceReg(total, Preplace);
    }
    total = total.join("\n");
    ...
    // rule 类型, 已处理完毕
    flag = 3;
    total = Rule_Handle(res_content.split("\n"), Pout0, Pin0).filter(Boolean);
  ...
  if (flag == 1) {
    ...
    if (total.length > 0) {
      ...
      if (Pcnt == 1) {
        $notify("解析后最终返回内容", "节点数量: " + total.split("\n").length, total);
      }
...
function SubFlow() {
  if (Pinfo == 1 && res_info) {
    var sinfo = res_info.replace(/ /g, "").toLowerCase();
    var total =
      "总流量: " +
...
function Type_Check(subs) {
  ...
  if (subs.indexOf(html) != -1 && res_link.indexOf("github.com" == -1)) {
    $notify("‼️ 该链接返回内容有误", "⁉️ 点通知跳转以确认链接是否失效", res_link, nan_link);
    type = "web";
  } else if (typeU == "nodes") {
    type = "Subs-B64Encode";
    ...
    // Clash 类型节点转换
    type = "Clash";
    res_content = Clash2QX(subs);
  } else if (
    (/hostname\=|pattern\=/.test(subs) || RewriteK.some(RewriteCheck)) &&
    !/\[(Proxy|filter_local)\]/.test(subs) &&
    para1.indexOf("dst=filter") == -1 &&
    subi.indexOf("securehostname") == -1 &&
    !/module|nodes/.test(typeU)
  ) {
    ...
    (DomainK.some(RuleCheck) || typeU == "domain-set") &&
    subs.indexOf("[Proxy]") == -1 &&
    typeU != "nodes"
  ) {
    type = "Rule";
    res_content = Domain2Rule(res_content); // 转换 domain-set
  } else if (subsn.length >= 1 && SubK2.some(NodeCheck) && !/\[(Proxy|filter_local)\]/.test(subs)) {
    //未b64加密的多行URI 组合订阅
    type = "Subs";
  } else if (SubK.some(NodeCheck)) {
    //b64加密的订阅类型
    type = "Subs-B64Encode";
  } else if (
    (subi.indexOf("tag=") != -1 &&
    ...
    res_content = Surge2QX(res_content).join("\n");
  } else if (
    ...
    type = "Subs";
  } else if (res_content.indexOf("server") != -1 && res_content.indexOf("server_port") != -1) {
    //SIP008
    //type = "QuanX"
    type = "Subs";
    res_content = SIP2QuanX(res_content);
  }
  // 用于通知判断类型，debug
  if (typeU == "X") {
    $notify(type, "", res_content);
  }
  return type;
}
...
function ToRaw(cnt) {
  ...
  var rawlink = res_link.replace("github.com", "raw.githubusercontent.com").replace("/blob", "");
  $notify(
    "⚠️⚠️ 将尝试解析该资源" + "⟦" + subtag + "⟧",
    "🚥 请正确使用GitHub的 raw 链接",
    "❌ 你的链接：" + res_link + "\n✅ 正确链接：" + rawlink,
...
function SCP2QX(subs) {
  ...
  for (var i = 0; i < subs.length; i++) {
    ...
    if (!NoteK.some(notecheck)) {
      if (SC.every(sccheck)) {
      ...
      } else if (subs[i].indexOf(" 302") != -1 || subs[i].indexOf(" 307") != -1) {
        //rewrite 302&307 复写
        ...
        rw =
          subs[i].split(" ")[0] +
          " url request-header ^GET " +
          pget +
          "(.+\\r\\n)Host:.+(\\r\\n) request-header GET " +
          pgetn +
          "$1Host: " +
          subs[i].split(" ")[1].split("://")[1].split(".com")[0] +
          ".com$2";
        nrw.push(rw);
...
function Rule_Handle(subs, Pout, Pin) {
  ...
  if (Tout != "" && Tout != null) {
    ...
    for (var i = 0; i < cnt.length; i++) {
      ...
      } else if (!RuleK.some(RuleCheck) && cc) {
        //if Pout.some, 不操作注释项
        dd = Rule_Policy(cc);
        if (Tin != "" && Tin != null) {
...
function VQ2QX(subs, Pudp, Ptfo, Pcert, Ptls13) {
  ...
  if (obfs.indexOf("obfs=over-tls") != -1 || obfs.indexOf("obfs=wss") != -1) {
    var cert =
      Pcert != 0 || subs.indexOf("allowInsecure=1") != -1
  ...
  if (obfs == "none") {
    ...
    var host =
      subs.indexOf("&obfsParam=") != -1
        ? "obfs-host=" + subs.split("&obfsParam=")[1].split("&")[0] + ", "
        : "";
    obfs = obfs + host;
  }
  if (obfs.indexOf("obfs=over-tls") != -1 || obfs.indexOf("obfs=wss") != -1) {
...
function TJ2QX(subs, Pudp, Ptfo, Pcert, Ptls13) {
  ...
  ptfo = Ptfo == 1 ? "fast-open=true" : "fast-open=false";
  tag =
    cnt.indexOf("#") != -1 ? "tag=" + decodeURIComponent(cnt.split("#")[1]) : "tag= [trojan]" + ip;
  ...
  for (var i = 0; i < ser0.length; i++) {
    if (ser0[i].indexOf("tag=") != -1) {
      ...
      var Lmoji = {
        ...
        "🇦🇹": ["奥地利", "Austria", "维也纳"],
        "🇦🇺": ["AU", "Australia", "Sydney", "澳大利亚", "澳洲", "墨尔本", "悉尼", "土澳"],
        "🇧🇪": ["BE", "比利时"],
        "🇧🇬": ["保加利亚", "Bulgaria"],
        "🇧🇷": ["BR", "Brazil", "巴西", "圣保罗"],
        "🇨🇦": [
        ...
        "🇨🇭": ["瑞士", "苏黎世", "Switzerland"],
        "🇨🇿": ["Czechia", "捷克"],
        "🇩🇪": ["DE", "German", "GERMAN", "德国", "德國", "法兰克福", "京德"],
        "🇩🇰": ["丹麦"],
        "🇪🇸": ["ES", "西班牙", "Spain"],
        "🇪🇺": ["EU", "欧盟", "欧罗巴"],
        "🇫🇮": ["Finland", "芬兰", "赫尔辛基"],
        ...
        "🇮🇱": ["Israel", "以色列"],
        "🇮🇳": ["India", "INDIA", "印度", "孟买", "Mumbai"],
        "🇰🇵": ["KP", "朝鲜"],
        "🇰🇷": ["KR", "Korea", "KOR", "韩国", "首尔", "韩", "韓"],
        "🇱🇻": ["Latvia", "Latvija", "拉脱维亚"],
        "🇲🇽️": ["MEX", "MX", "墨西哥"],
        "🇲🇾": ["MY", "Malaysia", "MALAYSIA", "马来西亚", "馬來西亞", "吉隆坡"],
...
function CV2QX(cnt) {
  ...
  node =
    "vmess=" + [ipt, pwd, mtd, udp, tfo, obfs, ohost, ouri, cert, tag].filter(Boolean).join(", ");
  //console.log(node)
  return node;
}
